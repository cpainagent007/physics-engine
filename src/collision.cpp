...
void CreateContacts(std::vector<Body>& bodies, std::vector<Contact>& contacts)
{
	for (int i = 0; i < bodies.size(); i++)
	{
		for (int j = i + 1; j < bodies.size(); j++)
		{
			...
			if (bodyA.bodyType != Body::BodyType::Dynamic && bodyB.bodyType != Body::BodyType::Dynamic) continue;
			...
			if (Intersects(bodyA, bodyB))
			{
				...
				{
					direction = Vector2{ Random::GetRandomFloat(-0.05f, 0.05f), Random::GetRandomFloat(-0.05f, 0.05f) * 0.5f };
				...
				contact.restitution = (bodyA.restitution + bodyB.restitution);
...
void ResolveContacts(std::vector<Contact>& contacts)
{
	for (auto& contact : contacts)
	{
		...
		contact.bodyA->AddForce(impulse, Body::ForceMode::Impulse);
		contact.bodyB->AddForce((impulse * -0.5f), Body::ForceMode::Impulse);
	}
}